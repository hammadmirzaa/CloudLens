...
import { 
  Search, Filter, Server, Activity, X, AlertCircle, HardDrive
} from 'lucide-react';
import { 
...
const Compute: React.FC = () => {
  usePageTitle('Compute Engine - CloudLens');
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 relative h-full">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Compute Engine</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-2 text-sm">Manage and monitor your virtual machine instances.</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 p-4 shadow-sm flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input 
            ...
            className="w-full bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-700 text-slate-900 dark:text-slate-200 rounded-lg pl-10 pr-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all"
          />
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm overflow-hidden">
        {isLoading ? (
          <div className="p-6 space-y-4 animate-pulse">
            {[1, 2, 3, 4, 5].map(i => (
              <div key={i} className="h-12 bg-gray-100 dark:bg-slate-800/50 rounded w-full"></div>
            ))}
          </div>
        ) : isError ? (
          <div className="p-12 text-center flex flex-col items-center">
            ...
            <p className="text-slate-500 dark:text-slate-400">Could not fetch compute data. Please try again.</p>
          </div>
        ) : filteredInstances.length === 0 ? (
          <div className="p-16 text-center flex flex-col items-center border-t border-gray-200 dark:border-slate-800/50">
            <Server className="w-16 h-16 text-slate-300 dark:text-slate-700 mb-4" />
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-800/60 text-slate-700 dark:text-slate-300">
                {filteredInstances.map((instance) => (
                  <tr key={instance.name} className="hover:bg-gray-50 dark:hover:bg-slate-800/30 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-slate-500 dark:text-slate-400">{instance.zone}</td>
                    <td className="px-6 py-4 text-slate-500 dark:text-slate-400">{instance.machineType}</td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col text-xs">
                        <span className="text-slate-700 dark:text-slate-300">{instance.internalIp} <span className="text-slate-400 dark:text-slate-600">(Int)</span></span>
                        {instance.externalIp ? (
                          <span className="text-slate-500 dark:text-slate-400">{instance.externalIp} <span className="text-slate-400 dark:text-slate-600">(Ext)</span></span>
                        ) : (
                          <span className="text-slate-400 dark:text-slate-600 italic">No external IP</span>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <button 
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white dark:bg-slate-800 hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-600 dark:hover:text-white text-slate-600 dark:text-slate-300 rounded text-xs font-medium transition-colors border border-gray-200 dark:border-slate-700 hover:border-blue-200 dark:hover:border-blue-500"
                      >
                        <Activity size={14} />
                        Metrics
                      </button>
...
const StatusBadge = ({ status }: { status: string }) => {
  if (status === 'RUNNING') {
    return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>RUNNING</span>;
  }
  if (status === 'TERMINATED') {
    return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400 border border-red-200 dark:border-red-500/20"><span className="w-1.5 h-1.5 rounded-full bg-red-500"></span>TERMINATED</span>;
  }
  return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20"><span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse"></span>STAGING</span>;
};
...
const MetricsDrawer = ({ vmName, onClose }: { vmName: string | null, onClose: () => void }) => {
  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-lg bg-white dark:bg-slate-900 border-l border-gray-200 dark:border-slate-800 z-50 shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        <div className="p-6 border-b border-gray-200 dark:border-slate-800 flex justify-between items-center bg-gray-50 dark:bg-slate-950/50">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Real-time performance metrics</p>
          </div>
          <button onClick={onClose} className="p-2 text-slate-400 hover:text-slate-700 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-slate-800 rounded-lg transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          {isLoading ? (
            <div className="space-y-8 animate-pulse">
              <div className="h-[250px] bg-gray-100 dark:bg-slate-800/50 rounded-xl border border-gray-200 dark:border-slate-700"></div>
              <div className="h-[250px] bg-gray-100 dark:bg-slate-800/50 rounded-xl border border-gray-200 dark:border-slate-700"></div>
            </div>
          ) : isError || !data ? (
            <div className="p-8 text-center bg-gray-50 dark:bg-slate-800/30 rounded-xl border border-gray-200 dark:border-slate-700/50 border-dashed">
              <AlertCircle className="w-10 h-10 text-red-400 mx-auto mb-3" />
              <p className="text-slate-600 dark:text-slate-300 font-medium">Failed to load metrics</p>
            </div>
          ) : (
            <>
              <div className="space-y-4">
                <h3 className="text-sm font-semibold text-slate-500 dark:text-slate-300 uppercase tracking-wider flex items-center gap-2">
                  <Activity size={16} className="text-blue-500 dark:text-blue-400" />
                  CPU Utilization (Last 1h)
                </h3>
                <div className="h-[250px] bg-gray-50 dark:bg-slate-950/50 rounded-xl border border-gray-200 dark:border-slate-800 p-4">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={data.cpu}>
                      <CartesianGrid strokeDasharray="3 3" stroke={theme === 'dark' ? '#334155' : '#e2e8f0'} vertical={false} />
                      <XAxis dataKey="time" stroke={theme === 'dark' ? '#64748b' : '#94a3b8'} tick={{ fontSize: 11 }} tickMargin={10} minTickGap={20} axisLine={false} tickLine={false} />
                      <YAxis stroke={theme === 'dark' ? '#64748b' : '#94a3b8'} tick={{ fontSize: 11 }} tickMargin={10} unit="%" axisLine={false} tickLine={false} />
                      <Tooltip 
                        contentStyle={{ backgroundColor: theme === 'dark' ? '#0f172a' : '#fff', borderColor: theme === 'dark' ? '#334155' : '#e2e8f0', color: theme === 'dark' ? '#f8fafc' : '#0f172a', borderRadius: '0.5rem', fontSize: '12px' }}
