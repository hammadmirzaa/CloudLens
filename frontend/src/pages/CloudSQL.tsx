    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-400 mt-1 text-sm">Manage relational databases (PostgreSQL, MySQL, SQL Server).</p>
        </div>
      </div>
      ...
      <div className="bg-slate-900 rounded-xl border border-slate-800 shadow-sm overflow-hidden">
        {isLoading ? (
          <div className="p-6 space-y-4 animate-pulse">
            {[1, 2, 3, 4].map(i => (
              <div key={i} className="h-16 bg-slate-800/50 rounded-lg w-full"></div>
            ))}
          </div>
        ) : isError ? (
          <div className="p-12 text-center flex flex-col items-center">
            ...
            <p className="text-slate-400">Could not fetch Cloud SQL data.</p>
          </div>
                  ...
                  return (
                    <tr key={instance.name} className="hover:bg-slate-800/30 transition-colors group">
                      ...
                      <td className="px-6 py-4 text-slate-400">
                        <span className="bg-slate-950 border border-slate-800 px-2 py-1 rounded text-xs">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1.5 w-40">
                          <div className="flex justify-between text-xs">
                            <span className="text-slate-400 flex items-center gap-1"><HardDrive size={12} /> {instance.storageUsedGb}GB</span>
                            <span className="text-slate-500">{instance.storageCapacityGb}GB</span>
                          </div>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="inline-flex items-center gap-1.5 text-slate-300 font-medium bg-slate-800/50 px-2 py-1 rounded">
                          <Activity size={14} className={instance.activeConnections > 0 ? "text-emerald-400" : "text-slate-500"} />
...
const StatusBadge = ({ status }: { status: string }) => {
  if (status === 'RUNNABLE') {
    return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"><CheckCircle2 size={12} /> Runnable</span>;
  }
  if (status === 'SUSPENDED') {
    return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-slate-500/10 text-slate-400 border border-slate-500/20"><PauseCircle size={12} /> Suspended</span>;
  }
  return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-amber-500/10 text-amber-400 border border-amber-500/20"><Settings size={12} className="animate-spin" /> Maintenance</span>;
};
