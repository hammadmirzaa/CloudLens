    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-400 mt-1 text-sm">Manage GKE clusters, nodes, and workloads.</p>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="h-48 bg-slate-900 rounded-xl border border-slate-800 animate-pulse"></div>
          ))}
        </div>
      ) : isError ? (
        <div className="bg-slate-900 rounded-xl border border-slate-800 p-12 text-center flex flex-col items-center">
          <AlertTriangle className="w-12 h-12 text-red-500 mb-4" />
          <h3 className="text-lg font-medium text-white mb-1">Error Loading Clusters</h3>
          <p className="text-slate-400">Could not fetch GKE data.</p>
        </div>
      ) : filteredClusters.length === 0 ? (
        <div className="bg-slate-900 rounded-xl border border-slate-800 p-16 text-center flex flex-col items-center">
          <Container className="w-16 h-16 text-slate-700 mb-4" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredClusters.map(cluster => (
            <div key={cluster.name} className="bg-slate-900 rounded-xl border border-slate-800 p-6 shadow-sm hover:border-slate-700 transition-colors relative overflow-hidden group">
              {cluster.status === 'ERROR' && <div className="absolute top-0 right-0 w-1 h-full bg-red-500"></div>}
              
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h3 className="font-semibold text-white text-lg flex items-center gap-2 group-hover:text-blue-400 transition-colors">
                  ...
                  <div className="flex items-center gap-3 mt-2 text-sm text-slate-400">
                    ...
                    <span>•</span>
                    <span className="font-mono text-xs text-slate-500">v{cluster.kubernetesVersion}</span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-8">
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-400 flex items-center gap-1.5"><Cpu size={14} /> CPU ({cluster.totalCpu} cores)</span>
                    <span className="text-white font-medium">{cluster.cpuUsagePercent}%</span>
                  </div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-400 flex items-center gap-1.5"><HardDrive size={14} /> Memory ({cluster.totalMemory}GB)</span>
                    <span className="text-white font-medium">{cluster.memoryUsagePercent}%</span>
                  </div>
                  <div className="w-full bg-slate-950 h-2 rounded-full overflow-hidden border border-slate-800">
                    <div 
...
const StatusBadge = ({ status }: { status: string }) => {
  if (status === 'RUNNING') {
    return <span className="flex items-center gap-1.5 text-emerald-400 text-xs font-medium bg-emerald-500/10 px-2.5 py-1 rounded-full border border-emerald-500/20"><CheckCircle2 size={12} /> Running</span>;
  }
  if (status === 'ERROR') {
    return <span className="flex items-center gap-1.5 text-red-400 text-xs font-medium bg-red-500/10 px-2.5 py-1 rounded-full border border-red-500/20"><AlertTriangle size={12} /> Error</span>;
  }
  return <span className="flex items-center gap-1.5 text-blue-400 text-xs font-medium bg-blue-500/10 px-2.5 py-1 rounded-full border border-blue-500/20"><RefreshCw size={12} className="animate-spin" /> Provisioning</span>;
};
